...
const loadCurriculum = () => {
  try {
    ...
    if (!raw) return deepCopy(DEFAULT_CURRICULUM);
    const saved = JSON.parse(raw);
    return {
      elementary: { ...deepCopy(DEFAULT_CURRICULUM.elementary), ...(saved.elementary || {}) },
      middle: { ...deepCopy(DEFAULT_CURRICULUM.middle), ...(saved.middle || {}) },
      high: { ...deepCopy(DEFAULT_CURRICULUM.high), ...(saved.high || {}) }
    };
  } catch (error) {
    console.error("커리큘럼 저장 내용을 읽는 중 문제가 발생했습니다.", error);
    return deepCopy(DEFAULT_CURRICULUM);
  }
};
      ...
      <div class="slide-inner">
        <div class="slide-copy">
          ...
          <div class="hero-actions">
            <a class="btn-primary" href="${slide.ctaLink || "index.html#contact"}">${slide.cta}</a>
          </div>
        </div>
      </div>
...
const renderTimetable = (data) => {
  ...
  list.innerHTML = data.timetable.map((item, index) => `
    <li>
      <button type="button" data-time="${index}">${item.name}</button>
    </li>
  `).join("");
};
...
const openSchedule = (index) => {
  ...
  panel.innerHTML = `
    <h3>${data.label} ${item.name} 시간표</h3>
    <p>${item.period}</p>
    <div class="schedule-table-wrap">
    <table class="schedule-table">
      <thead><tr><th>요일</th><th>시간</th><th>수업</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    </div>
  `;
