...
// 지금 화면에 쓰는 공유 저장 내용입니다.
let memory = { v: 6, updatedAt: 0, homepage: null, curriculum: null };
...
const pushRemote = async () => {
  ...
  const payload = {
    ...
    curriculum: typeof window.getCurriculumStore === "function"
      ? window.getCurriculumStore()
      : memory.curriculum
  };
  const headers = {
  ...
  try {
    let sha = "";
    const meta = await fetch(`${REPO_API_URL}?ref=main`, { headers, cache: "no-store" });
    if (meta.ok) {
      const info = await meta.json();
      sha = info && info.sha ? info.sha : "";
    }
    const body = {
      message: "학원 사이트 내용을 저장합니다.",
...
const initAdmin = () => {
  ...
  if (saveBtn) {
    saveBtn.addEventListener("click", async () => {
      ...
      try {
        const synced = await pushRemote();
        closeAdmin();
        if (synced) {
          showToast("저장했습니다. PC와 휴대폰에 함께 반영됩니다.");
        } else if (!getToken()) {
