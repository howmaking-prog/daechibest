...
const mergeCurriculum = (saved) => ({
  elementary: { ...deepCopy(DEFAULT_CURRICULUM.elementary), ...((saved && saved.elementary) || {}) },
  middle: { ...deepCopy(DEFAULT_CURRICULUM.middle), ...((saved && saved.middle) || {}) },
  high: { ...deepCopy(DEFAULT_CURRICULUM.high), ...((saved && saved.high) || {}) }
});
  ...
  const blocks = Array.from({ length: TIMETABLE_SLOT_COUNT }, (_, i) => {
    ...
    return `
      <div class="time-block">
        ...
        <div class="field"><label>기간</label><input name="time${n}Period" placeholder="3월 – 6월" /></div>
        ${rows}
      </div>
    `;
  }).join("");
...
const fillTimetableAdmin = (form, data) => {
  ...
  (data.timetable || []).forEach((item, i) => {
    ...
    if (form.elements[`time${n}Period`]) form.elements[`time${n}Period`].value = item.period || "";
    for (let r = 1; r <= TIMETABLE_ROW_COUNT; r += 1) {
      const row = (item.rows && item.rows[r - 1]) || ["", "", ""];
      for (let c = 1; c <= 3; c += 1) {
...
const readTimetableAdmin = (form, pageData) => {
  ...
  next.timetable = next.timetable.map((item, i) => {
    const n = i + 1;
    if (!form.elements[`time${n}Name`]) return item;
    const rows = [];
    for (let r = 1; r <= TIMETABLE_ROW_COUNT; r += 1) {
    ...
    return {
      ...
      period: form.elements[`time${n}Period`].value.trim() || item.period,
      rows: rows.length ? rows : item.rows
    };
  });
  return next;
};
